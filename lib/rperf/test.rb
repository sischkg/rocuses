  ...
  class TestCase
    ...
    def generate_popen_mock( args )
      ...
      args.each { |command,output_lines|
        ...
        output.should_receive( :gets ).with_any_args.and_return( *output_lines, nil )
    ...
    def generate_readable_mock( args )
      ...
      args.each { |path,result|
        mock.should_receive( :readable? ).with( path ).and_return( result )
      }
      return mock
    end
    ...
    def generate_read_mock( args ) 
      ...
      args.each { |path,output_lines|
        ...
        output.should_receive( :gets ).with_no_args.and_return( *output_lines )
