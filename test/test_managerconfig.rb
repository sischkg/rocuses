...
class LoadManagerConfigXMLTest < Test::Unit::TestCase
  ...
  must "check_default_values" do
    ...
    assert_equal( '/var/rocuses/graph', config.graph_directory )
...
<rocuses>
  <manager>
    <options>
      ...
      <graph directory="/usr/local/rocuses/graph"/>
    </options>
  </manager>
</rocuses>
END_XML
    ...
    assert_equal( '/usr/local/rocuses/graph', config.graph_directory )
    assert_equal( 500,                        config.image_width )
    assert_equal( 120,                        config.image_height )
  end
end
